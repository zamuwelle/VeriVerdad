		...
		<>
			<div className="min-h-screen flex flex-col lg:flex-row bg-[var(--color-bg)] text-[var(--color-text)]">
				...
				<main className="flex-1 md:px-32 pt-16">
					<h1 className="text-2xl font-bold text-[var(--color-text)] tracking-tight mb-8 text-center">History</h1>
					...
					{ ! Array.isArray(chats) || chats.length === 0 ? <p className='text-center'>No recent conversation yet</p> :
						...
						return (
							<div key={chat.id} className={`group flex items-center justify-between px-3.5 py-2 hover:opacity-80 p-2 md:px-16 border border-[var(--color-border)] ${isChatActive ? 'bg-[var(--color-bg)] text-[var(--color-text)] font-semibold' : 'text-[var(--color-text)] bg-transparent font-normal'}`}>
								{editingId === chat.id ? (
